  ...
  return (
    <div
      className={cn(
        ...
        isLanding ? 'md:right-20 md:-top-[52px] right-0 -top-20' : 'right-0 -top-[42px]',
      )}
    >
      <motion.button
      ...
      >
        <ArrowLeft size={20} className={`${prevBtnDisabled ? 'text-gray-600' : 'text-gray-700'}`} />
      </motion.button>
      <motion.button
      ...
      >
        <ArrowRight size={20} className={`${nextBtnDisabled ? 'text-gray-600' : 'text-gray-700'}`} />
      </motion.button>
    </div>
  );
};
