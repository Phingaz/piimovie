...
function Calendar({ className, classNames, showOutsideDays = true, ...props }: React.ComponentProps<typeof DayPicker>) {
  return (
    <DayPicker
      ...
      className={cn('p-3 bg-gray-900 rounded-sm', className)}
      classNames={{
        months: 'flex flex-col sm:flex-row gap-2 ',
        month: 'flex flex-col gap-4 bg-gray-900',
        caption: 'flex justify-center pt-1 relative items-center w-full text-gray-400',
        caption_label: 'text-sm font-medium',
        nav: 'flex items-center gap-1',
        nav_button: cn(
          buttonVariants({ variant: 'outline' }),
          'size-7 bg-transparent p-0 opacity-80 hover:opacity-100 text-gray-300',
        ),
        ...
        day: cn(
          ...
          'size-8 p-0 font-normal aria-selected:opacity-100 hover:bg-gray-800 hover:text-gray-300',
        ),
